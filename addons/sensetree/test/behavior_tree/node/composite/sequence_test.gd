...
var sequence_composite: SenseTreeSequenceComposite
...
func before_test() -> void:
	...
	sequence_composite = auto_free(sequence_composite_script.new())
	...
	sensetree.add_child(sequence_composite)
...
func test_parameterized_child_index (child_a_status: int, child_b_status :int, child_c_status :int, expected_status :int, test_parameters := [
	...
	[0, 0, 2, 2], [1, 0, 2, 1], [2, 0, 2, 2],
	[0, 1, 1, 1], [1, 1, 1, 1], [2, 1, 1, 2],
	[0, 1, 2, 1], [1, 1, 2, 1], [2, 1, 2, 2],
	[0, 2, 1, 2], [1, 2, 1, 1], [2, 2, 1, 2],
	[0, 2, 2, 2], [1, 2, 2, 1], [2, 2, 2, 2] ])  -> void:
	...
	sequence_composite.add_child(child_mock_a)
	sequence_composite.add_child(child_mock_b)
	sequence_composite.add_child(child_mock_c)
...
func test_get_sensenode_class() -> void:
	...
	# When
	var result_class_name = sequence_composite.get_sensenode_class()
