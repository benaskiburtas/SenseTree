...
const INPUT_PORT_TYPE = SenseTreeConstants.GraphNodePortType.INPUT
const OUTPUT_PORT_TYPE = SenseTreeConstants.GraphNodePortType.OUTPUT
...
func _load_node_icon(sense_node: SenseTreeNode) -> void:
	...
	if not icon_path or icon_path.is_empty():
		push_warning("Could not find matching icon for SenseTree node of type %s." % sense_node_class)
		return
